...
class JMail extends PHPMailer
{
	 ...
	 * Send the mail
	 *
	 * @return  boolean|  Boolean true if successful, boolean false if the `mailonline` configuration is set to 0 or sending the message fails.
	 *
	 * @since   11.1
	...
	public function Send()
	{
		if (JFactory::getConfig()->get('mailonline', 1))
		{
			...
			{
                throw new RuntimeException(JText::_('JLIB_MAIL_FUNCTION_DISABLED'));
			}
			...
			if ($result == false)
			{
                throw new RuntimeException(JText::_($this->ErrorInfo));
			}
